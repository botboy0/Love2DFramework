...
describe("NullTransport", function()
	it("Transport.Null.new() returns a table", function()
		local null = Transport.Null.new()
		assert.is_table(null)
	end)

	it("mark_networkable() does not error", function()
		local null = Transport.Null.new()
		assert.has_no.errors(function()
			null:mark_networkable("some_event")
		end)
	end)

	it("is_networkable() always returns false", function()
		local null = Transport.Null.new()
		null:mark_networkable("some_event")
		assert.is_false(null:is_networkable("some_event"))
		assert.is_false(null:is_networkable("other_event"))
	end)

	it("queue() does not error", function()
		local null = Transport.Null.new()
		assert.has_no.errors(function()
			null:queue("some_event", { x = 1 })
		end)
	end)

	it("send() does not error", function()
		local null = Transport.Null.new()
		assert.has_no.errors(function()
			null:send({ event = "test", data = {} })
		end)
	end)

	it("flush() does not error", function()
		local null = Transport.Null.new()
		assert.has_no.errors(function()
			null:flush()
		end)
	...
	it("receive() returns nil", function()
		local null = Transport.Null.new()
		assert.is_nil(null:receive())
	end)

	it("receive_all() returns empty table", function()
		local null = Transport.Null.new()
		local msgs = null:receive_all()
		assert.is_table(msgs)
		assert.are.equal(0, #msgs)
	end)
end)
